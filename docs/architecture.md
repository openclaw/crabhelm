...
The AWS backend runs the portable HTTP and control-plane services as one long-lived Node.js task on ECS/Fargate. An internet-facing Application Load Balancer terminates HTTPS, applies OIDC authentication to the console host, and forwards the separately authenticated runtime host with native WebSocket support. The server verifies the signed ALB identity assertion, expected load-balancer signer, OIDC client, issuer, expiry, and a single explicit verified-email claim before resolving a principal, then maps configured emails and groups to the administrator role. Boolean `true` and Cognito's exact UserInfo string `"true"` are accepted; missing, false, invalid, or multi-valued claims are rejected. An unauthenticated console-only logout rule forwards `/logout` and `/signed-out`; the application expires the ALB base cookie and all four shards before redirecting to the no-store landing page. The OIDC provider remains responsible for organization admission and upstream session revocation.
...
Slack uses one organization app when the installation's canonical Slack mode is on. The edge verifies Slack's timestamped HMAC signature over the raw body before parsing it, resolves the requester through Slack, and routes only through an administrator-approved workspace/channel persona binding. Slack-off installations inject no Slack credentials, expose no Slack ingress route, terminally fail queued Slack delivery without an outbound request, and report the integration as not configured. The runtime never receives Slack credentials.
