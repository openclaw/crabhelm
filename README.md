...
- The agent workspace installs an outbound nftables allowlist (loopback, established replies, DNS, NTP, DHCP, and TCP 443 only) that drops the cloud instance-metadata endpoints before any credential is written and reapplies before networking after reboot, so a compromised agent cannot exfiltrate over arbitrary ports or read substrate credentials. Set `CRABHELM_EGRESS_LOCKDOWN=required` to fail provisioning when the allowlist cannot be enforced and boot-persisted, or `off` to remove the managed table and boot unit.
