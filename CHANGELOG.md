...
- Lock down agent-workspace egress: the bootstrap installer applies and boot-persists an outbound nftables allowlist (loopback, replies, DNS, NTP, DHCP, TCP 443) that drops instance-metadata endpoints before credentials are written. Configurable via `CRABHELM_EGRESS_LOCKDOWN` (`attempt` default, `required`, `off`); disabling removes the managed table and boot unit.
